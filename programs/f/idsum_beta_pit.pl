...
my (%sumhash, %modsumhash, $database, %dbhash,$pepXML_mode);
...
#			if ($$hash{'protein'} =~ /^Random__/ || $$hash{'protein'} =~ /^Decoy__/){ $bad++;
	...
	my %fprhash;
	my ($numofproteins) = $utils->create_dbHash($database,\%dbhash);
	#%dbhash = %$tempdbhash;
...
#				next if ($protein =~ /^Decoy/ || $protein =~ /^Random/);
		...
		if (1)
		{
			...
			my $ldaresult=$idutils->rLDA_analysis($LDAfile,$paramhash{'search_engine'});
...
#=head #for debug
	...
	if ($paramhash{'one_hit_wonders_removal'} > 0){
		...
		for my $protein (keys %tempproteinhash){
			...
			next if ($tempproteinhash{$protein}{'total'} > 1);
		...
		printf "Removing %d one-hit wonders\n", scalar(keys %onehitdels);
...
=cut
	...
	while (1)
	{
		...
		if ($printResults)
		{
		printf "\nTesting FDR for target outfiles (not FDR for total outfiles, might take minutes): %.4f%% (= decoys/targets)\n",$min_fpr;
		printf LOGFILE "\nTesting FDR for target outfiles (not FDR for total outfiles, might take minutes): %.4f%% (= decoys/targets)\n",$min_fpr;
		}
...
sub update_grouphash
{
	...
	if (!defined($paramhash{pit_file})) { die "pit file not defined!!!\nPlease update your parameter file.\n"; }
	my ($grouphash,$decoy_grouphash)=build_grouphash($paramhash{pit_file});
        my %best_proteins;
        assignBestProtein2Peptide($peptidehash, $grouphash, $decoy_grouphash, \%best_proteins); #print scalar(keys %best_proteins),"\n";
        assignOrder2group($proteinhash, $grouphash, \%best_proteins);
        assignOrder2group($proteinhash, $decoy_grouphash, \%best_proteins);
        my ($group_num, $subgroup_num)=get_group_num($grouphash);
        my ($decoy_group_num, $decoy_subgroup_num)=get_group_num($decoy_grouphash);
